...
template <typename T>
void print_array_to_file(T array, int N){
	fstream fout("nqueen_solutions.txt", ios::out | ios::trunc);
	for (int i=0; i<N; i++){
		for (int j=0; j<N; j++){
			fout << array[i][j] << " ";
		}
...
fstream fstack("nqueen_stack.txt", ios::out | ios::trunc);
bool nQueen ( int** arr, int Row, int n){
	...
	for(int col=0; col<n; col++){
		fstack << "\n" << setfill(' ') << right << setw(4*col) << "(" << Row << ", " << col << ", " << n << ")";
		if(isSafe(arr,Row,col,n)) {
			print_array_to_file(arr, n);
//			fstack << "\n(" << Row << ", " << col << ", " << n << ")";
