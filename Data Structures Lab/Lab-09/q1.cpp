...
class BinaryRoot{
	...
	public:
		...
		bool DispatcherTest(Node* r){
			if (r == NULL)
				return true;
			if (r->leftChild != NULL && r->rightChild != NULL){
				if (r->getKey() < r->leftChild->getKey())
					return false;
				if (r->getKey() > r->rightChild->getKey())
					return false;
				return true;
			}
			if (DispatcherTest(r->leftChild) == false)
				return false;
			if (DispatcherTest(r->rightChild) == false)
				return false;
			return true;
		}
...
int main(){
	...
	if (TreeX.DispatcherTest(TreeX.getRoot())){
		cout << "Allowed";
	}
	else{
		cout << "Rejected";
